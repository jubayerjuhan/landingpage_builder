...
const LayoutDropZone: React.FC<{ position: 'above' | 'below'; layoutId?: string }> = ({
  ...
  return (
    <div ref={setNodeRef} className={`${styles.layoutDropZone} ${isOver ? styles.active : ''}`}>
...
const LayoutContainer: React.FC<{
  ...
  const { elements, selectedElementId, selectElement, reorderLayout, isPreviewMode } =
    useBuilderStore();
  ...
  return (
    <div className={styles.layoutWrapper}>
      ...
      >
        ...
        {showDragIndicator && !isPreviewMode && (
          <div className={styles.dragOverIndicator}>Drop here to add layout</div>
        )}

        <div className={styles.layoutContent} style={getElementStyles(layout, viewportMode)}>
          {rows.map(row => (
          ...
          {columnElements.map((element, index) => (
            <div key={element.id} className={styles.elementWrapper}>
              ...
              {index === columnElements.length - 1 && (
                <ElementDropZone position="below" targetElementId={element.id} isLast={true} />
              )}
            </div>
          ))}
        </div>
